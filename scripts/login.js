...
// 🆕 Matching robusto de matrícula em xpData
...
// 🆕 Extrai array de conquistas visíveis de um campo do Firebase (formato wrapper)
function extrairConquistasVisiveis(cv) {
  if (!cv) return null;
  // Formato novo: { mostraTodas: bool, ativos: { id1: true, ... } }
  ...
  // Compat: array direto (versão antiga)
  if (Array.isArray(cv)) return cv.slice();
  return null;
}

// 🆕 Monta payload para salvar conquistasVisiveis
function montarConquistasVisiveisPayload(selecionadas) {
  if (selecionadas === null) return null; // deleta → mostra todas
  if (!Array.isArray(selecionadas) || selecionadas.length === 0) {
    return { mostraTodas: false }; // nenhuma visível
  }
  const ativos = {};
...
function renderizarSeletorConquistasVisiveis() {
  ...
  // Se `null`, significa "sem config" → considera todas as desbloqueadas como visíveis
  ...
  grid.querySelectorAll(".conquista-visivel-item:not(.bloqueada)").forEach((el) => {
    el.addEventListener("click", () => {
      ...
      // Inicializa a lista efetiva se estiver em "modo null"
...
// ==========================================
// 🆕 CONQUISTAS NO MODAL DE PERFIL (CORRIGIDO)
// ==========================================
async function carregarConquistasNoModalPerfil(matricula) {
  ...
  try {
    ...
    // 🆕 Usa o extrator robusto (wrapper {mostraTodas, ativos} ou array antigo)
    ...
    if (conquistasVisiveis !== null) {
      // Configuração explícita: mostra só as selecionadas E desbloqueadas
      ...
      // Sem config: mostra todas as desbloqueadas
  ...
  get(itemRef).then((snapshot) => {
    ...
    if (recado) {
      ...
      if (confirm("Excluir este recado?")) {
        remove(itemRef).then(() => exibirToast("Recado excluído.", "sucesso"))
          .catch((err) => exibirToast("Erro: " + err.message, "erro"));
      }
    }
  });
  ...
  return get(ref(db, "perfis_alunos/" + matricula))
    .then((snap) => {
      ...
      const perfilFinal = {
        ...
        // 🆕 mantém o objeto wrapper como veio do Firebase
...
window.abrirModalEditarPerfil = function () {
  ...
  // 🆕 Carrega preferência de conquistas visíveis (formato wrapper)
  const cv = perfilUsuarioAtual.conquistasVisiveis;
  const listaExtraida = extrairConquistasVisiveis(cv);
  conquistasVisiveisSelecionadas = listaExtraida; // null = mostrar todas; array = customizada
  renderizarSeletorConquistasVisiveis();
...
async function salvarPerfilEditado() {
  ...
  const perfilAtualizado = {
    ...
    // 🆕 Formato wrapper — evita o Firebase deletar quando lista vazia
    conquistasVisiveis: montarConquistasVisiveisPayload(conquistasVisiveisSelecionadas),
  };
  // Se for null, precisamos remover explicitamente (update com null funciona como "deletar")
  const btnSalvar = document.getElementById("btn-salvar-perfil");
...
// ==========================================
// SALA DOS PROFESSORES (🆕 CORRIGIDO: Em Risco + conquistas por matching robusto)
// ==========================================
async function carregarSalaProfessores() {
  ...
  console.log("[sala] Exemplo matrícula:", Object.keys(perfis)[0], "| Exemplo XP:", Object.keys(xpData)[0]);
  ...
  const alunos = Object.keys(perfis)
    ...
    .map((mat) => {
      const p = perfis[mat] || {};
      const r = resumos[mat] || {};
      // 🆕 Usa matching robusto para localizar o XP
