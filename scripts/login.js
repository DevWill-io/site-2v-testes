    ...
    listaEl.innerHTML = r.admins.map((a) => {
      ...
      return `
        <div class="adm-item">
          ...
          <button type="button" class="adm-item-btn" onclick="removerAdmin('${a.matricula}', '${escaparHTML(a.nome)}')" ${euMesmo ? "disabled" : ""}>
...
async function carregarLogsAdm() {
  ...
  try {
    ...
    logsEl.innerHTML = r.logs.map((log) => {
      const data = new Date(log.timestamp).toLocaleString("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" });
      let texto = "";
      ...
      return `<div class="adm-log-item"><span class="adm-log-texto">${texto}</span><span class="adm-log-data">${data}</span></div>`;
    }).join("");
  } catch (e) {
...
window.carregarLogsAdm = carregarLogsAdm;
...
window.adicionarAdmin = async function () {
  ...
  try {
    const r = await chamarAPI("/api/admin?tipo=adicionar", { matricula, acao: "adicionar" });
    if (!r.sucesso) throw new Error(r.erro);
...
window.removerAdmin = async function (matricula, nome) {
  ...
  try {
    const r = await chamarAPI("/api/admin?tipo=adicionar", { matricula, acao: "remover" });
    if (!r.sucesso) throw new Error(r.erro);
