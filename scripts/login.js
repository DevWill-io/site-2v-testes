...
// ==========================================
// ESTADO GLOBAL DO USUÁRIO, BANCOS E FILTROS
// ==========================================
...
// ==========================================
// FUNÇÕES AUXILIARES / SEGURANÇA / TOASTS
// ==========================================
...
// Converte URLs no texto em links clicáveis seguros
...
// Sistema de Notificação Toast (Substitui alerts invasivos)
...
function nomeParaExibicao(nome) {
  var partes = String(nome || "Usuário")
    .trim()
    .split(/\s+/);
...
function parseDuracaoParaHoras(valor) {
  ...
  if (str.endsWith("h")) {
    horas = parseFloat(str.replace("h", ""));
  } else if (str.endsWith("d")) {
    horas = parseFloat(str.replace("d", "")) * 24;
  } else {
    horas = parseFloat(str) * 24;
  }
...
// ==========================================
// 2. LISTENERS DO FIREBASE EM TEMPO REAL
// ==========================================

// Escuta Mural de Recados
...
onValue(perfisRef, (snapshot) => {
  ...
  console.log(
    "[perfis_alunos] carregados:",
    bancoDePerfis.length,
    bancoDePerfis
  );
...
// ==========================================
// 3. ENVIO DE RECADOS COM SUPORTE A ANEXOS
// ==========================================
...
// ==========================================
// 4. MÉTODOS GLOBAIS (ANEXADOS AO WINDOW)
// ==========================================
...
// Exibir Modal de Perfil (com bio, redes e debug)
window.abrirModalPerfil = function (identificador) {
  console.log("[abrirModalPerfil] chamado com:", identificador);

  const modal = document.getElementById("modal-perfil");
  if (!modal) {
    console.warn("[abrirModalPerfil] #modal-perfil não existe no DOM");
    return;
  }
  ...
  console.log("[abrirModalPerfil] perfil encontrado?", !!perfil, perfil);
  ...
  const nomeExibir =
    perfil.nomeCompleto || perfil.nome || "Não informado";
  const fotoExibir =
    perfil.foto ||
  ...
  if (matEl)
    matEl.textContent = perfil.matricula || perfil.id || "Não informada";
  if (acessoEl) {
  ...
  if (redesEl) {
    ...
    if (links.length > 0) {
      redesEl.innerHTML = links
        .map(
          (l) =>
            `<a href="${l.url}" target="_blank" rel="noopener noreferrer" title="${l.titulo}" aria-label="${l.titulo}" class="modal-rede-link">
               <i class="${l.icone}"></i>
             </a>`
        )
        .join("");
...
window.renderizarMural = function () {
  ...
  recadosReversos.forEach((recado) => {
    ...
    const btnExcluir =
      ehAutor || ehAdmin
        ? `<button type="button" class="btn-like" style="color: #ff4757;" onclick="excluirRecado('${recado.id}')" title="${
            ehAdmin && !ehAutor ? "Excluir como Admin" : "Excluir meu recado"
          }">
    ...
    const btnEditar =
      ehAutor || ehAdmin
        ? `<button type="button" class="btn-like" style="color: #eccc68;" onclick="editarRecado('${recado.id}')" title="${
            ehAdmin && !ehAutor ? "Editar como Admin" : "Editar meu recado"
          }">
    ...
    const anexoHTML = recado.link_anexo
      ? `<div class="recado-anexo" style="margin-top: 8px;">
           <a href="${recado.link_anexo}" target="_blank" rel="noopener noreferrer" style="font-size: 0.85em; color: #70a1ff; text-decoration: underline;">
    ...
    listaComentarios.forEach((com) => {
      ...
      htmlComentarios += `
        <div class="comentario-item" style="display:flex; justify-content:space-between; align-items:center; background: rgba(255,255,255,0.05); padding: 6px 10px; border-radius: 6px; margin-top: 5px; font-size: 0.85em;">
          <div style="flex:1;">
            <strong>${escaparHTML(com.autor_nome)}:</strong> ${converterLinks(
    ...
    div.innerHTML = `
          <div class="recado-header">
              ...
              <span class="recado-data" title="${textoExpiracao}">
                  <i class="fa-regular fa-clock" style="font-size: 0.85em; margin-right: 3px;"></i>${
                    recado.data
                  }${tagEditado} • <small style="opacity: 0.8;">${textoExpiracao}</small>
              </span>
          </div>
      ...
      recado.id
                  ...
                  <i class="${iconeCoracao}"></i> ${totalLikes}
              </button>
...
// Renderizar Perfis dos Alunos com Filtro
window.renderizarPerfis = function () {
  const container = document.getElementById("lista-perfis");

  console.log("[renderizarPerfis] chamado");
  console.log("  container existe?", !!container);
  console.log("  bancoDePerfis.length:", bancoDePerfis.length);
  console.log("  filtroPerfilTexto:", JSON.stringify(filtroPerfilTexto));

  if (!container) {
    console.warn("[renderizarPerfis] #lista-perfis não encontrado no DOM!");
    return;
  }
  ...
  console.log("  perfisFiltrados.length:", perfisFiltrados.length);
      ...
      <p class="sem-perfis" style="text-align:center; opacity:0.7; padding:20px; grid-column: 1 / -1;">
        ${
          bancoDePerfis.length === 0
            ? "Nenhum membro cadastrado ainda. Os alunos aparecerão aqui após fazerem login."
  ...
  perfisFiltrados.forEach((perfil, idx) => {
    ...
    const nomeExibir =
      perfil.nomeCompleto || perfil.nome || "Usuário sem nome";
    ...
    card.innerHTML = `
      <div class="perfil-avatar">
        <img src="${escaparHTML(
          fotoFinal
        )}" alt="${escaparHTML(nomeExibir)}" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=${encodeURIComponent(
      nomeExibir
    )}&background=random'" />
      </div>
      <div class="perfil-info">
        <h4 class="perfil-nome">${escaparHTML(nomeExibir)}</h4>
        <span class="perfil-matricula">${escaparHTML(
  ...
  console.log("  → total de cards renderizados:", container.children.length);
};
...
window.forceLogout = function () {
  ...
  document.cookie =
    "suapToken=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
  document.cookie =
...
// ==========================================
// 4.5. EDIÇÃO DE PERFIL (Base64 + Firebase + localStorage)
// ==========================================
  ...
  return get(ref(db, "perfis_alunos/" + matricula))
    .then((snap) => {
      ...
      const perfilFinal = {
        ...
        foto:
          dados.foto || (local && local.foto) || window.usuarioLogado.foto,
...
window.abrirModalEditarPerfil = function () {
  ...
  document
    .querySelectorAll(
      "#form-editar-perfil input, #form-editar-perfil textarea"
    )
...
async function salvarPerfilEditado() {
  ...
  if (inputFoto && inputFoto.dataset.restaurar === "1") {
    ...
    fotoFinal = perfilUsuarioAtual.foto || window.usuarioLogado.foto;
  }

  const nome = document.getElementById("edit-nome").value.trim();
  if (!nome) {
    exibirToast("Informe um nome de exibição.", "erro");
    return;
  }
...
function atualizarStatusNotas(mensagem, erro) {
  var status = document.getElementById("notas-status");
  if (!status) return;
  status.textContent = mensagem;
  status.classList.toggle("nota-reprovada", Boolean(erro));
}
...
function obterEtapasDaDisciplina(disciplina) {
  ...
  var possuiNotaSegundoSemestre = [3, 4].some(function (numeroEtapa) {
    ...
    return (
      formatarNota(etapa && typeof etapa === "object" ? etapa.nota : etapa) !==
      null
    );
  });
...
function renderizarNotas(disciplinas) {
  ...
  if (!disciplinas.length) {
    corpo.innerHTML =
      '<tr><td colspan="5" class="notas-vazia">Nenhuma disciplina encontrada neste período.</td></tr>';
    return;
  }
  ...
  disciplinas.forEach(function (disciplina) {
    ...
    grupos[configuracao.tipo].push({
      disciplina: disciplina,
      etapas: configuracao.etapas,
    });
  });
  ...
  ["Semestral", "Anual"].forEach(function (tipo) {
    ...
    grupos[tipo].forEach(function (item) {
      ...
      var notasEtapas = item.etapas.map(function (numeroEtapa) {
        var etapa = disciplina["nota_etapa_" + numeroEtapa];
        return formatarNota(
          etapa && typeof etapa === "object" ? etapa.nota : etapa
        );
      });
      var notasPreenchidas = notasEtapas.filter(function (nota) {
        return nota !== null;
      });
      var soma = notasPreenchidas.reduce(function (total, nota) {
        return total + nota;
      }, 0);
      var mediaApi = formatarNota(disciplina.media_disciplina);
      var media = notasPreenchidas.length
        ? soma / notasPreenchidas.length
        : mediaApi;
      var totalEtapas = item.etapas.length;
      var faltantes = totalEtapas - notasPreenchidas.length;
      var necessaria =
        faltantes > 0 ? (meta * totalEtapas - soma) / faltantes : null;
      var projecao =
        faltantes > 0
          ? necessaria <= 0
            ? "Meta atingida"
            : "Precisa de " + textoNota(necessaria)
          : media !== null && media >= meta
            ? "Aprovado"
            : "Abaixo da meta";
      var classeMedia =
        media !== null && media >= meta ? "nota-aprovada" : "nota-atencao";
      if (necessaria !== null && necessaria > 100)
        classeMedia = "nota-reprovada";

      var linha = document.createElement("tr");
      [
        disciplina.disciplina ||
          disciplina.codigo_diario ||
          "Disciplina sem nome",
        notasEtapas.map(textoNota).join(" | "),
        textoNota(media),
        disciplina.numero_faltas === undefined
          ? "-"
          : disciplina.numero_faltas,
        projecao,
      ].forEach(function (valor, indice) {
        var celula = document.createElement("td");
        celula.textContent = valor;
        if ((indice === 2 || indice === 4) && classeMedia)
          celula.className = classeMedia;
...
function carregarBoletim(ano, periodo) {
  atualizarStatusNotas("Buscando notas no SUAP...");
  suap.getAuthenticatedResource(
    ...
    function (resposta) {
      ...
      atualizarStatusNotas(
        (resposta.count || (resposta.results || []).length) +
          " disciplina(s) carregada(s)."
      );
    },
    function (xhr) {
      renderizarNotas([]);
      atualizarStatusNotas(
        "Não foi possível carregar o boletim (HTTP " + xhr.status + ").",
        true
      );
    }
  );
  ...
  suap.getAuthenticatedResource(
    ...
    function (resposta) {
      ...
      if (periodos.length)
        ...
        atualizarStatusNotas(
          "Nenhum período letivo disponível no SUAP.",
          true
        );
    },
    function (xhr) {
      atualizarStatusNotas(
        "Não foi possível carregar os períodos (HTTP " + xhr.status + ").",
        true
      );
    }
  );
  ...
  if (suap.isAuthenticated()) {
    ...
    if (calendarEl && typeof FullCalendar !== "undefined") {
      var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events:
          "acb20a08d58749d48304dbda5c87bfb7f0671483ecc4ed942683ad5a1307e78d@group.calendar.google.com",
    ...
    suap.getResource(scope, function (dados_suap) {
      var fotoPath =
        dados_suap.url_foto_150x200 ||
        dados_suap.url_foto_75x100 ||
        dados_suap.foto ||
        "";
      var fotoUrl = "";
      ...
      if (fotoPath) {
        if (fotoPath.startsWith("http://") || fotoPath.startsWith("https://")) {
          ...
          fotoUrl =
            fotoBaseUrl + (fotoPath.startsWith("/") ? "" : "/") + fotoPath;
        }
      } else {
        fotoUrl =
